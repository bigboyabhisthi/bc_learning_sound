...
def main():
    ...
    for fs in fs_list:
        convert_fs(
            os.path.join(data_path, "UrbanSound8K/audio"),
    ...
    for fold in sorted(os.listdir(src_path)):
        if os.path.isdir(os.path.join(src_path, fold)):
            ...
            for src_file in sorted(glob.glob(os.path.join(src_path, fold, "*.wav"))):
                ...
                subprocess.call(
                    "ffmpeg -i {} -ac 1 -ar {} -acodec pcm_s16le -loglevel error -y {}".format(
...
def create_dataset(src_path, dst_path):
    ...
    for fold in range(1, 11):
        ...
        for wav_file in sorted(
            glob.glob(os.path.join(src_path, "fold{}".format(fold), "*.wav"))
        ):
